...
const DEFAULT_LOCAL_DIR = path.join(DATA_DIR, 'catalog');
...
class FileCatalogClient {
  ...
  activeRoots() {
    ...
    if (config.local.enabled) {
      roots.push({ kind: 'local', path: config.local.path || DEFAULT_LOCAL_DIR });
    }
  ...
  ensureDefaultLayout(rootDir = DEFAULT_LOCAL_DIR) {
  ...
  async downloadMod(slug, serverId = null, fileKind = '') {
    if (!this.isConfigured()) {
      throw new Error('File catalog is not configured');
    }
    const mod = this.getMod(slug, fileKind);
    if (!mod) throw new Error('Mod not found in the file catalog');
    const packPaths = (mod.filePaths || []).filter((filePath) => filePath && fs.existsSync(filePath));
    if (!packPaths.length) {
  ...
  resolveLocalPath(raw) {
    const value = String(raw || '').trim() || DEFAULT_LOCAL_DIR;
...
fileCatalog.ensureDefaultLayout();
